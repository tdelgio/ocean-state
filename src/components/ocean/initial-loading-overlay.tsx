...
export function InitialLoadingOverlay() {
  const [visible, setVisible] = useState(() => {
    ...
    if (window.sessionStorage.getItem("ocean-state-initial-loading-seen") === "true") {
      return false;
    }
    window.sessionStorage.setItem("ocean-state-initial-loading-seen", "true");
    return true;
  });
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[radial-gradient(circle_at_70%_10%,rgba(91,231,255,0.18),transparent_35%),radial-gradient(circle_at_20%_80%,rgba(20,184,166,0.10),transparent_35%),linear-gradient(180deg,#F7FCFD_0%,#EDF8F7_100%)] px-6 text-[#102b3a] dark:bg-[radial-gradient(circle_at_70%_10%,rgba(91,231,255,0.10),transparent_35%),radial-gradient(circle_at_20%_80%,rgba(20,184,166,0.08),transparent_35%),linear-gradient(180deg,#071723_0%,#0b2230_100%)] dark:text-[#f4fbff]">
      ...
      <section className="relative w-full max-w-sm rounded-[1.6rem] border border-[#094c60]/10 bg-white/70 p-6 text-center shadow-[0_20px_60px_rgba(7,35,45,0.10)] backdrop-blur-xl dark:border-white/12 dark:bg-[#0b2230]/72">
        <div className="mx-auto grid size-16 place-items-center rounded-full border border-[#094c60]/12 bg-white/75 text-[#0d5968] shadow-[0_12px_28px_rgba(7,35,45,0.08)] dark:border-white/14 dark:bg-[#102a3a] dark:text-[#9debf9]">
